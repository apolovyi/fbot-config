...
class BB_RPB_TSL_V1(IStrategy):
    ...
    def informative_1h_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_1h['bb_width'] = (
                (informative_1h['bb_upperband2'] - informative_1h['bb_lowerband2']) / informative_1h[
            'bb_middleband2'])
        ...
        is_clucHA = (
                ...
                (
                        (
                                ...
                                (dataframe['bb_delta_cluc'] > dataframe[
                                    'ha_close'] * self.buy_clucha_bbdelta_close.value) &
                                (dataframe['ha_closedelta'] > dataframe[
                                    'ha_close'] * self.buy_clucha_closedelta_close.value) &
                        ...
                        (
                                ...
                                (dataframe['ha_close'] < self.buy_clucha_close_bblower.value * dataframe[
                                    'bb_lowerband2'])
                        )
                )
        )
...
def pmax(df, period, multiplier, length, MAtype, src):
    ...
    #     final_ub[i] = basic_ub[i] if (
    #             (basic_ub[i] < final_ub[i - 1])
    #             | (mavalue[i - 1] > final_ub[i - 1])) else final_ub[i - 1]
    #     final_lb[i] = basic_lb[i] if (
    #             (basic_lb[i] > final_lb[i - 1])
    #             | (mavalue[i - 1] < final_lb[i - 1])) else final_lb[i - 1]
