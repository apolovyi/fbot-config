...
class MyStrategy03_STRAT(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                    (dataframe["high"] < dataframe["high"].shift(1)) & (dataframe["low"] > dataframe["low"].shift(1))
            ), 'candle_type'] = "1"
        dataframe.loc[
            (
                    (dataframe["high"] < dataframe["high"].shift(1)) & (dataframe["low"] < dataframe["low"].shift(1))
        ...
        # if (dataframe["high"] < dataframe["high"].shift(1)) & (dataframe["low"] > dataframe["low"].shift(1)):
        #     dataframe['candle_type'] = "1"
        # if (dataframe["high"] < dataframe["high"].shift(1)) & (dataframe["low"] < dataframe["low"].shift(1)):
        #     dataframe['candle_type'] = "2_down"
        # if (dataframe["high"] > dataframe["high"].shift(1)) & (dataframe["low"] > dataframe["low"].shift(1)):
        #     dataframe['candle_type'] = "2_up"
        # if (dataframe["high"] > dataframe["high"].shift(1)) & (dataframe["low"] < dataframe["low"].shift(1)):
        #     dataframe['candle_type'] = "3"

        print(dataframe["candle_type"])

        dataframe['candle_green'] = dataframe["open"] <= dataframe["close"]
        dataframe['candle_red'] = dataframe["open"] > dataframe["close"]
