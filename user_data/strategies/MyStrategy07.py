...
class MyStrategy07(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['sma_m'] = ta.SMA(dataframe, timeperiod=89)
        dataframe['sma_l'] = ta.SMA(dataframe, timeperiod=55)
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_signal_1:
            conditions = [
                qtpylib.crossed_above(dataframe['sma_s'], dataframe['sma_m']),
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.sell_signal_1:
            conditions = [
                qtpylib.crossed_below(dataframe['sma_s'], dataframe['sma_m']),
                # dataframe["low"] < dataframe["s1_ema_xxl"],
                # dataframe["close"] > dataframe["s1_ema_xxl"],
                # qtpylib.crossed_above(dataframe["s1_ema_sm"], dataframe["s1_ema_md"]),
                # dataframe["s1_ema_xs"] < dataframe["s1_ema_xl"],
